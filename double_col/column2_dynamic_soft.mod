...
param lkmin1; 
param vtr{-1..K-1} ; 
param termepsk{0..K-1} ; 
param descepsk{0..K-1} ; 
param ltr{-1..K-1};
param normx{0..K}; #norm of states
param sigcosttest{1..2,1..7};
param mult{1..7};
param sig_val{1..7};
param normz{0..nfe};
param x1_init{1..NT,1..2};
param M1_init{1..NT};
param x2_init{1..NT,1..2};
param M2_init{1..NT};
param lecss := -.223;
param time;
...
# Disturbances
param F{fe} ; #feed
param  qF{fe} ; #liquid fraction in feed
param  pV{fe} ; #energy price
param zF {fe,1..3} ; # feed composition
...
#soft constraints
x1lower {i in 1..NT, j in 1..2, f in fe}:x1_0[i,j,f]>=0-x1eps[i,j,f];
M1lower {i in 1..NT, f in fe}:M1_0[i,f]>=.25-M1eps[i,f];
M1upper {i in 1..NT, f in fe, c in cp}:M1_0[i,f]<=.75+M1eps[i,f];
...
#soft constraints
x2lower {i in 1..NT, j in 1..2, f in fe}:x2_0[i,j,f]>=0-x2eps[i,j,f];
M2lower {i in 1..NT, f in fe}:M2_0[i,f]>=.25-M2eps[i,f];
M2upper {i in 1..NT, f in fe}:M2_0[i,f]<=.75+M2eps[i,f];
...
const5 {i in NF..NT-1,f in fe}:
    V1[i,f] = VB1[f] + (1-qF[f])*F[f] ; # vapor flow above feed
...
const15{f in fe, c in cp}:
    M1dot[NF,f,c]=L1[NF+1,f,c] - L1[NF,f,c] + V1[NF-1,f] - V1[NF,f] + F[f];
    
    const15_r{ c in cp}:
    M1dot_r[NF,c]=L1_r[NF+1,c] - L1_r[NF,c] + V1[NF-1,0] - V1[NF,0] + F_r;

const16{f in fe, c in cp}:
    Mx1dot[NF,1,f,c]=L1[NF+1,f,c]*x1[NF+1,1,f,c] - L1[NF,f,c]*x1[NF,1,f,c] + V1[NF-1,f]*y1[NF-1,1,f,c] - V1[NF,f]*y1[NF,1,f,c] + F[f]*zF[f,1];
...
const17{f in fe, c in cp}:
    Mx1dot[NF,2,f,c]=L1[NF+1,f,c]*x1[NF+1,2,f,c] - L1[NF,f,c]*x1[NF,2,f,c] + V1[NF-1,f]*y1[NF-1,2,f,c] - V1[NF,f]*y1[NF,2,f,c] + F[f]*zF[f,2];
...
x1_r_Lag{i in 1..NT,j in 1..2,  c in cp}: x1_r[i,j,c] = x1_0[i,j,0] + h[0]* sum{k in cp} omega[k,c]*x1dot_r[i,j,k];
...
M1_r_Lag{i in 1..NT, c in cp}: M1_r[i,c] = M1_0[i,0] + h[0]* sum{k in cp} omega[k,c]*M1dot_r[i,k];
...
x2_r_Lag{i in 1..NT,j in 1..2, c in cp}: x2_r[i,j,c] = x2_0[i,j,0] + h[0]* sum{k in cp} omega[k,c]*x2dot_r[i,j,k];
...
M2_r_Lag{i in 1..NT, c in cp}: M2_r[i,c] = M2_0[i,0] + h[0]* sum{k in cp} omega[k,c]*M2dot_r[i,k];
...
termcon : sum {i in 1..NT}(sum{j in 1..2}((x1_0[i,j,nfe]-x1ref[i,j])^2 +(x2_0[i,j,nfe]-x2ref[i,j])^2 )) + sum {i in 1..NT}( (M1_0[i,nfe]-M1ref[i])^2 + (M2_0[i,nfe]-M2ref[i])^2) <=    termeps ; 
...
# OBJECTIVE FUNCTION
#--------------------
var cost{f in fe}=pF*F[f] + pV[f]*(VB1[f] + VB2[f])-pA*D1[f] - pB*D2[f] - pC*B2[f];

var penalty_2{f in fe}=D1eps[f] +B1eps[f]+D2eps[f] +B2eps[f];
var penalty_M_TC{f in fe}=sum{i in 1..NT}(M1eps[i,f] +M2eps[i,f]+TC1eps[i,f]+TC2eps[i,f]);
var penalty_x_y{f in fe}=x2ceps[f]+sum{i in 1..NT}sum{j in 1..2}(x1eps[i,j,f]+x2eps[i,j,f]);
...
var regu_1{f in fe}=F_w*(F[f]-Fref)^2 +qF_w*(qF[f]-qFref)^2+pV_w*(pV[f]-pVref)^2+VB1_w*(VB1[f]-VB1ref)^2 +VB2_w*(VB2[f]-VB2ref)^2+LT1_w*(LT1[f]-LT1ref)^2 +LT2_w*(LT2[f]-LT2ref)^2;
...
var penalty{f in fe}=(penalty_2[f]+penalty_M_TC[f]+penalty_x_y[f]+penalty_V_L[f]);

minimize obj: rho*pen_switch*(termeps+desceps)+sum{f in fe}(econ_switch*cost[f]+rho*pen_switch*penalty[f]+regu_frac*regu[f]+track_switch*tracking[f]);
