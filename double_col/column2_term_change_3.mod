...
param K :=150;
param econ_switch := 1;
param track_switch := 0;
param noise_switch := 1;
param regu_frac:= 0;
param term_cost_switch := 0 ;
param Nmax:=25;
...
var termcost = rho_psi*sum{i in s, j in s} stat_vec[i]*P[i,j]*stat_vec[j] ; 
var tracking {f in fe} = (1/rho_L)*(cont_w*((VB1[f]-VB1ref)^2 + (VB2[f]-VB2ref)^2+ (LT1[f]-LT1ref)^2 + (LT2[f]-LT2ref)^2+(D1[f]-D1ref)^2 +(B1[f]-B1ref)^2+(D2[f]-D2ref)^2 + (B2[f]-B2ref)^2) + state_w*(sum {i in 1..NT}((M1_0[i,f]-M1ref[i])^2 +(M2_0[i,f]-M2ref[i])^2+sum{j in 1..2}( (x1_0[i,j,f]-x1ref[i,j])^2 +(x2_0[i,j,f]-x2ref[i,j])^2  ) )  ));
...
#endpt : sum {i in 1..NT}(sum{j in 1..2}((x1_0[i,j,nfe]-x1ref[i,j])^2 +(x2_0[i,j,nfe]-x2ref[i,j])^2 )) + sum {i in 1..NT}( (M1_0[i,nfe]-M1ref[i])^2 + (M2_0[i,nfe]-M2ref[i])^2) <=  termeps ; 
...
# 2 norm
descent_con1: lk =tracking[0] ;

descent_con2: vk= termcost+sum{f in fe} tracking[f]; 
...
var regu_VBLT{f in fe}=VB1_w*(VB1[f]-VB1ref)^2 +VB2_w*(VB2[f]-VB2ref)^2+LT1_w*(LT1[f]-LT1ref)^2 +LT2_w*(LT2[f]-LT2ref)^2;
var regu_DB{f in fe}=D1_w*(D1[f]-D1ref)^2 +B1_w*(B1[f]-B1ref)^2+D2_w*(D2[f]-D2ref)^2 +B2_w*(B2[f]-B2ref)^2;
var regu_M{f in fe}=sum{i in 1..NT}(M1_w[i]*(M1_0[i,f]-M1ref[i])^2 +M2_w[i]*(M2_0[i,f]-M2ref[i])^2);
var regu_x{f in fe}=sum{i in 1..NT}(sum{j in 1..2}(x1_w[i,j]*(x1_0[i,j,f]-x1ref[i,j])^2 +x2_w[i,j]*(x2_0[i,j,f]-x2ref[i,j])^2));
var regu{f in fe}=regu_VBLT[f]+regu_DB[f]+regu_M[f]+regu_M[f]+regu_x[f];
...
minimize obj: econ_switch *sum{f in fe} cost[f]+term_cost_switch*termcost+rho*pen_switch*(termeps+desceps)+sum{f in 0..nfe}rho*pen_switch*penalty2[f]+sum{f in fe}(rho*pen_switch*penalty1[f]+track_switch*tracking[f]+regu_frac*regu[f]);