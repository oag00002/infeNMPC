...
### Revised by Xue Yang on Sept. 8, 2014
### Discrete steady state problem 
...
x1_ss{i in 1..NT, j in 1..2}: x1_0[i,j,1]=x1[i,j,nfe,ncp];
##x1_constr{i in 1..NT, j in 1..2,f in fe}:x1_0[i,j,f]=x1[i,j,f,ncp];
...
##M1_constr{i in 1..NT,f in fe}:M1_0[i,f]=M1[i,f,ncp];
M1_ss{i in 1..NT}: M1_0[i,1]=M1[i,nfe,ncp];
...
x2_ss{i in 1..NT, j in 1..2}: x2_0[i,j,1]=x2[i,j,nfe,ncp];
##x2_constr{i in 1..NT, j in 1..2,f in fe}:x2_0[i,j,f]=x2[i,j,f,ncp];
...
##M2_constr{i in 1..NT,f in fe}:M2_0[i,f]=M2[i,f,ncp];
M2_ss{i in 1..NT}: M2_0[i,1]=M2[i,nfe,ncp];
...
var cost=sum{f in fe} (pF*F[f] + pV[f]*(VB1[f] + VB2[f])+h[f]*sum{c in cp}(-pA*D1[f,c] - pB*D2[f,c] - pC*B2[f,c])*omega[c,ncp]);
minimize obj: cost;
