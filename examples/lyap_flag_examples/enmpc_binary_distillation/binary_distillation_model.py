"""
eNMPC binary distillation column — economic objective maximizing purity.

Same 42-tray Methanol/n-Propanol column as ``binary_distillation``, but with
an economic stage cost that maximizes bottoms purity while penalizing
reboiler heat duty and condenser duty:
    stage_cost = -(100*(1 - x[1,t]) - 10*Qr[t] + Qc[t])

MVs:  Qr — reboiler heat duty, Rec — reflux ratio
...
def variables_initialize(m):
    """
    Declare all Pyomo variables, sets, and parameters for the eNMPC distillation column.
