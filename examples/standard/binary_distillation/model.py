"""
Binary distillation column — Methanol/n-Propanol separation (setpoint tracking).

42-tray column. States: M[tray] (holdup), x[tray] (mole fraction).
MVs:  Qr — reboiler heat duty (MJ/h), Rec — reflux ratio
CVs:  x[1] — bottoms composition, T[29] — mid-column temperature
DV:   xf — feed mole fraction
Objective: quadratic tracking to steady-state setpoints.

Note: ``custom_objective`` should be ``False`` for this model.
      The ``custom_objective_rule`` stub is kept for structural compatibility.
"""
...
def variables_initialize(m):
    """
    Declare all Pyomo variables, sets, and parameters for the distillation column.
    ...
    # ---- Setpoints for CVs ----
    m.setpoints = pyo.Param(m.CV_index, initialize={"x[1]": 0.01677068292481846, "T[29]": 352.92248738561443})
...
def custom_objective_rule(m, options):
    if hasattr(m, "obj"):
        delattr(m, "obj")
