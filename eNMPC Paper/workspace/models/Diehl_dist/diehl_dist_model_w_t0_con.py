...
# ---------------------------------------------------------------------------------------------------------------------
def dist_daemodel(dynamic = True, nfe = 20, th = 60):
    mod = ConcreteModel()
    ...
    if dynamic:
        ...
        mod.Mdot = Var(mod.t, mod.tray, initialize=0.0)
...
def set_ics(mod, ic_states):
    for i in mod.tray:
        mod.x_ic[i] = ic_states["x"][i]
        mod.M_ic[i] = ic_states["M"][i]

def initial_w_ics(mod, ic_states):
    for i in mod.t:
        for j in mod.tray:
