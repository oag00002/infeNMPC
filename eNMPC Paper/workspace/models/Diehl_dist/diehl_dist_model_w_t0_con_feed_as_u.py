...
def de_M_rule(m, i, k):
    if 1 < k < m.Ntray:
        return m.Mdot[i, k] == \
               (m.V[i, k - 1] - m.V[i, k] + m.L[i, k + 1] - m.L[i, k] + m.feed[i,k])
    elif k == 1:
        return m.Mdot[i, 1] == \
...
# ---------------------------------------------------------------------------------------------------------------------
def dist_daemodel(nfe = 20, th = 60, dynamic=True):
    mod = ConcreteModel()
    ...
    #                   mutable=True)
    mod.feed = Var(mod.t, mod.tray,
                   initialize=lambda m, i, k: 57.5294 if k == 21 else 0.0,
                   )
    mod.feed[...].fix()
    mod.feed[:,21].unfix()
...
def create_model_hard_bounds(mod):
    state_bounds = {
                    ...
                    "hv": (1.0, 1e+07),
                    "Qc": (0.0, 1e+08),
                    "D": (0.0, 1e+04), # try 25 but fail QAQ
                    "Vm": (0.0, 1e+04),
    ...
    # Doesn't use these var but some annoying warnings pop out when the model is shifted
    mod.Mv[:, 1].setlb(None)
    mod.Mv[:, 1].setub(None)
    mod.Mv[:, 42].setlb(None)
    mod.Mv[:, 42].setub(None)
