...
#####################################
#### This file has noise on Feed ####
has_feedNoise = True
regenerate_noise = True
read_noise_from_file = False

if regenerate_noise:
    # noise_std_precent = 0.03
    noise_ratio = 0.02
if read_noise_from_file:
    noise_source_dir = ""
#####################################

save_result_dir = \
    "/home/khl/KHL/Distillation_column/eNMPC_sc_gradLagran/Case17_L2EndpointPenalty_feedNoise"
...
save_milestone_sol_folder = join(save_result_dir, "milestone_sols")
if not exists(save_milestone_sol_folder):
    print("Create a folder to save the solution at milestone iteration!")
    makedirs(save_milestone_sol_folder)
...
if has_feedNoise:
    if (regenerate_noise and read_noise_from_file):
        raise RuntimeError("regenerate_noise and read_noise_from_file are both true and the same time!")
    orig_feed = plant.feed[21].value
    if regenerate_noise:
        # feedNoise = {
        #     "feedNoise": random.normal(loc=orig_feed, scale=noise_std_precent*orig_feed, size=120).tolist()
        #     }
        feedNoise = {
            "feedNoise": random.uniform(low=orig_feed-noise_ratio/2*orig_feed, 
                                        high=orig_feed+noise_ratio/2*orig_feed,
                                        size=120).tolist()
            }
        save_dict_in_json(feedNoise, join(save_result_dir, "feedNoise.json"))
    if read_noise_from_file:
        with open(join(noise_source_dir, "feedNoise.json"), "r") as fr:
            feedNoise = json.load(fr)
...
for step in range(1, iterations+1):
    ...
    if has_feedNoise:
        cur_feedNoise = feedNoise["feedNoise"][step-1]
        plant.feed[21].set_value(cur_feedNoise)
        
        print("")
        print("step ", step)
        print("current feed noise = ", plant.feed[21].value)
        print("Noise difference = ", plant.feed[21].value - orig_feed)
    ...
    if step%10 == 0:
        ...
        for comp in olnmpc.component_data_objects(Var):
            ...
            val = comp.value
            var_info[key] = val
        save_dict_in_json(var_info,
                          join(save_milestone_sol_folder, vars_filename))

        # save curr_states
        curr_states_filename = "curr_state_iter" + str(step+1) + ".json"
        save_dict_in_json(curr_states,
                          join(save_milestone_sol_folder, curr_states_filename))
        ...
        save_dict_in_json(Lya_info,
                          join(save_milestone_sol_folder, Lya_info_filename))
