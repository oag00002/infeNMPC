...
def variables_initialize(m):
    ...
    # ---- Controlled Variable Expressions ----
    # Create expressions for CVs to make them accessible by simple names
    # A purity in Column 1 distillate (top of column 1)
    def xD1A_rule(m, t):
        return m.x1[41, 1, t]
    m.xD1A = pyo.Expression(m.time, rule=xD1A_rule)

    # B purity in Column 2 distillate (top of column 2)
    def xD2B_rule(m, t):
        return m.x2[41, 2, t]
    m.xD2B = pyo.Expression(m.time, rule=xD2B_rule)

    # C purity in Column 2 bottoms (bottom of column 2)
    def xC_rule(m, t):
        return 1 - m.x2[1, 1, t] - m.x2[1, 2, t]
    m.xC = pyo.Expression(m.time, rule=xC_rule)

    # ---- Setpoints for CVs (product purities) ----
    m.setpoints = pyo.Param(m.CV_index, initialize={
        "xD1A": 0.95,   # A purity in D1
        "xD2B": 0.95,   # B purity in D2
